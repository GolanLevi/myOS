...
graph TD
    ...
    C -->|"פעולה בטוחה\n(לדוגמה, ספאם)"| D("✅ מבצע אוטומטית\n(העברה לאשפה)")
    
    C -->|"פעולה רגישה\n(לדוגמה, תיאום/שליחה)"| E["⛔ BREAKPOINT\nהגרף מושהה"]
    
    E --> F["💬 כרטיס אישור בטלגרם\nמציג: טיוטה, מועד, ולוז יומי"]
    
    F --> G{"👤 פעולת משתמש\n(כפתורי Inline)"}
    
    G -->|"אשר"| H["▶️ הגרף ממשיך (Resumes)"]
    G -->|"דחה"| I("❌ הפעולה מבוטלת הפעולה")
    G -->|"הכוונה ידנית"| J["✍️ הסוכן מנסח\nטיוטה חדשה מבוססת הנחיה"]
    
    H --> K("✅ מפעיל את הכלי\n(המייל נשלח / האירוע נוצר ביומן)")
    J --> E
```
